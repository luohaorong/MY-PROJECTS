		...
		<div class="configure_left_head">
			<div class="configure_head_content clearfix">
				...
				<div class="head_content_right float_left">
					共设{{ $provinces->count() }}个区域，还有{{ $provinces->count() - $agencies->count() }}个无管理人员
				</div>
			</div>
		</div>
			...
			<table class="left_body_table">
				...
				<tbody>
					...
					<tr class="tbabel_row" data-uuid="{{ $province->uuid }}">
						<td class="tbody_content">AP{{ $province->code }}</td>
						<td class="tbody_content">{{ $province->name }}</td>
						<td class="tbody_content">{{ $agencies->get($province->uuid)->name ?? '' }}</td>
						<td class="tbody_content">******</td>
						<td class="tbody_content">{{ $agencies->get($province->uuid)->real_name ?? '' }}</td>
						<td class="tbody_content">{{ $agencies->get($province->uuid)->id_card ?? '' }}</td>	
						<td class="tbody_content">{{ $agencies->get($province->uuid)->mobile ?? '' }}</td>
						<td class="tbody_content">{{ $agencies->has($province->uuid) ? \Carbon\Carbon::parse($agencies->get($province->uuid)->agent_started_at)->format('Y年m月d日') : '' }}</td>	
						<td class="tbody_content">{{ $agencies->has($province->uuid) ? \Carbon\Carbon::parse($agencies->get($province->uuid)->agent_ended_at)->format('Y年m月d日') : '' }}</td>	

						@if ($agencies->has($province->uuid) && $agencies->get($province->uuid)->status === config('const.agencies_status_normal'))
						<td class="tbody_content normal">正常</td>	
						@elseif ($agencies->has($province->uuid) && $agencies->get($province->uuid)->status === config('const.agencies_status_locked'))
						<td class="tbody_content stop">停用</td>
						@elseif (!$agencyAreas->has($province->uuid))
						<td class="tbody_content empty">空缺</td>
						@elseif ($agencyAreas->has($province->uuid) && is_null($agencyAreas->get($province->uuid)))
...
<script type="text/javascript">
	...
	window.global.store = "{{ action('ProvinceAgencyController@store') }}";
	window.global.history = "{{ action('ProvinceAgencyController@history') }}";
	window.global.close = "{{ action('ProvinceAgencyController@close') }}";
	window.global.open = "{{ action('ProvinceAgencyController@open') }}";
</script>
@endsection
