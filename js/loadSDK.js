...
(function(window, undefind) {
			...
			if(isBlob){
				...
				obj.onreadystatechange = function() {
					if(obj.readyState === 4 && (obj.status === 200 || obj.status == 304)) {
						fn && fn.call(this, obj.response); //从服务器获得数据
					}
				};
			}else{
				obj.open("GET", newUrl, true);
	...
	//获取图片src
	dataSDK.prototype.getImgSrc = function(name,fn){
		var This = this;
		var result = this.request;
		result.onsuccess = function(e) {
			var db = e.target.result;
		...
		implement: function(data,img,fn) {
			...
			if(oldV != +data.version) {
				...
				data.dataList.map(function(item, index) {
					...
					var oldVersion = oldVersionData.dataList[index] ? +oldVersionData.dataList[index].version : 0;
