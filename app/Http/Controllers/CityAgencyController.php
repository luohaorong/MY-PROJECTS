...
class CityAgencyController extends Controller
{
    /**
     * 显示列表页面
     *
    ...
    public function index(Request $request)
    {
        ...
        $cities = Area::where('parent_uuid', $area->uuid)
                        ->orderBy('lft')
                        ->get();

        $uuids = $cities->map(function ($item) {
            return $item->uuid;
        });
        ...
        return view('agency.city', [ 'provinces' => $provinces, 'cities' => $cities, 'agencyAreas' => $agencyArea, 'agencies' => $agencies, 'wechatUsers' => $wechatUsers ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $agency->agent_type = config('const.agencies_agent_type_city');
        $agency->status = config('const.agencies_status_normal');
