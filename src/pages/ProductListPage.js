...
class ProductListPage extends React.Component{
	...
	componentDidMount(){
		...
		let country=bee.getQueryString('country');
		let This=this;
		let filter = bee.getQueryString('filter');
		let filterArr = [];
		filterArr.push(filter)
		filter ? filterArr = JSON.stringify(filterArr) : filterArr = '';
		filter ? bee.cache('filterUuid',filterArr) : bee.cache('filterUuid',[])
				...
				if(data.error_code){
					...
					case 'putao':
					  data.data.map(function(item){
							if(item.latest==='true'){
								let filterUuid;
								JSON.parse(bee.cache('filterUuid')).length ? filterUuid = bee.cache('filterUuid') : filterUuid = '';
								filterUuid ? '' : This.setState({loadUuid:item.uuid});
								bee.cache('category',item.uuid);
								}
							});
					  break;
					case 'jiushipin':
	...
	postParent(dataKey,filter,isPrice){
		...
		let isUp;
		let filterUuid;
		bee.cache('filterUuid')&&JSON.parse(bee.cache('filterUuid')).length ? filterUuid = bee.cache('filterUuid') : filterUuid = '';
		this.setState({
			filter:filter,
			isPrice:isPrice
		});
		if(dataKey){
			filterUuid ? goodsUuid='' : goodsUuid=dataKey;
			bee.cache('category',dataKey);
			this.setState({
				reTabState:true
			})
		}else{
