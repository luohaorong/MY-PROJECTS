...
bee.link = {
	...
	'uploadImg':'http://admin.zongjiuhui.com/upload',
...
bee.getCode = function (origin) {
	var codeUrl=bee.link.weixin + '/JumpPage&origin=' + origin;
	var queryString = {
		appid: 'wx500d1d09ee93a851',
		redirect_uri: encodeURIComponent(bee.link.cloud + '/wechat/redirectUri?ru=' + codeUrl),
		response_type: 'code',
...
bee.post = function (url, data, fn, isSign,isFile) {
	...
	if(isFile !== undefined && isFile){
		...
		obj.onreadystatechange = function() {
			...
			if(obj.response == '' && obj.readyState == 4){
					var loghost = 'http://wechat.luo.com/error/report?';
			        var img = new Image(0,0);
			        var locUrl = window.location.href;
			        var _url =[];
			        _url.push('name=post');
			        _url.push('message'+'='+encodeURIComponent('服务器未响应'));
			        _url.push('location'+'='+encodeURIComponent(locUrl));
			        _url.push('line=undefined');
			        _url.push('func=undefined');
			        var url = _url.join('&');
			        img.src = loghost+url;
			    }
...
bee.isSelected = function(arr){
	...
	if(len){
		arr.map(function(item){
			tmp.push(item.some(function(i){
				return i;
			}))
		})
		isSubmint = tmp.every(function(j){
			return j;
		});
	}else{
		isSubmint = false;
...
bee.pushUrl = function(){
	var allUrl = window.location.href;
	var host = window.location.host;
	var afterHost = allUrl.split(host);
	var sessionUrl = JSON.parse(bee.cache('Urlstate')) || [];
	var index = sessionUrl.indexOf(afterHost[1]);
	if(index === -1){
		sessionUrl.push(afterHost[1]);
	}else{
		sessionUrl.splice(index,1);
		sessionUrl.push(afterHost[1]);
	}
	bee.cache('Urlstate',sessionUrl);
}
