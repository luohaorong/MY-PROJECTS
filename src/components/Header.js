...
class Header extends React.Component {
	...
	componentDidMount(){
		let middleImg=this.props.middleImg;
		if(middleImg){
			let dataKey=this.refs['list'].getAttribute('data-key');
				...
				<ul className='selectList' style={this.state.selectListStyle}>
					{
						headerListContent.map(function (item) {
							...
							return 	<li ref='list' key={item.id} data-key={item.id} onClick={This.listChange} className='listItem'>{item.textList}</li>;
