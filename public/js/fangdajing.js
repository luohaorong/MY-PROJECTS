	...
	a.fn.etalage = function(b) {
		...
		a.each(this, function() {
			...
			if(aD.is("ul") && aD.children("li").length && aD.find("img.etalage_source_image").length) {
				var aa, Z, O, G, an, s, f, aO, aI, at, aN = aD.attr("class"),
				...
				if(p > 1 || !c.smallthumb_hide_single) {
					...
					a.each(ar, function() {
						...
						var img_src=O.lastIndexOf('/');
						var new_src=O.substring(0,img_src);
						a('<li><img class="etalage_small_thumb" src="' +new_src+'/'+ c.small_img_size+ '" /></li>').data("thumb_id", G).appendTo(l)
					});
