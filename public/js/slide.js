			...
			$.fn.extend({
				tyslide:function(opts){
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						var w2=boxsount*(prams.controlsW+prams.spacing*boxsount);//此宽度是controls的宽度
						//var left2=((boxwidth-w2)/2);  //计算的控制按钮的x坐标 居中
						var left2=1300;//自定义位置
						...
						$(item).find(".controls>li").text("").css({"width":prams.controlsW,"height":prams.controlsH,"border-radius":prams.radius,"padding":"0",'margin-left':prams.spacing,"display":"block","border":"0"});
...
//									console.log("执行完毕");
						...
						$(item).hover(function(){
							for(var i=0;i<timer_arr.length;i++){
								clearInterval(timer_arr[i]);//鼠标移动到pptbox盒子上面的时候，清理掉定时器，轮播不再切换
							}
						},function(){
