...
//								var img_preview = document.getElementById('wine_lable_small');
								...
								img_box.setAttribute('data_name', '自定义酒标');
								...
								img1.setAttribute('data_src',e.target.result);

								img_box.appendChild(img1);
								img_box.appendChild(img2);
//								tempDivPreview.prepend(img_box);
								$('#wine_label_check').prepend(img_box)


							}
							reader.readAsDataURL(fileObj.files[0]);
							$('.upload_wine_label_limit').text('酒标尺寸：750*1334px，大小不超过6M')
						}
					} else {

						$('#wine_label_check:first-child').remove();
						$('.upload_wine_label_limit').removeAttr('data-success').text('图片过大（必须小于6M）,请重选').css('color', '#EA0000');
					}
					...
					if(browserVersion.indexOf("MSIE 6") > -1) { //ie6
						...
						var tempDivPreview = document.getElementById(divPreviewId);
							...
							img1.setAttribute("data_src", fileObj.value);
						var img2 = document.createElement('img');
						...
						if(filesize <= 6) {
							...
							if(browserVersion.indexOf("MSIE 9") > -1) {
								...
								newPreview.setAttribute("class", 'wine_label_check_box');
								newPreview.setAttribute("data_name", '自定义酒标');
								newPreview.style.width = 77 + "px";
								newPreview.style.height = 79 + "px";
...
//						tempDivPreview.parentNode.insertBefore(newPreview, tempDivPreview);
						$(tempDivPreview).prepend(newPreview);
						$(newPreview).append($('<img id="wine_lable_small"  src='+document.selection.createRange().text+ '/>'))
						$(newPreview).append($('<img class="wine_label_selected" data_src="/img/wine_selected.png"  src="/img/wine_selected.png" />'))
//						tempDivPreview.style.display = "none";
					...
					if(firefoxVersion < 7) { //firefox7以下版本
						...
						document.getElementById(imgPreviewId).setAttribute("data_src", fileObj.files[0].getAsDataURL());
					} else { //firefox7.0+                    
						document.getElementById(imgPreviewId).setAttribute("src", window.URL.createObjectURL(fileObj.files[0]));
						document.getElementById(imgPreviewId).setAttribute("data_src", window.URL.createObjectURL(fileObj.files[0]));
					}
				} else {
					document.getElementById(imgPreviewId).setAttribute("src", fileObj.value);
					document.getElementById(imgPreviewId).setAttribute("data_src", fileObj.value);
				}
			} else {
				if(allowExtention.indexOf(extention) <= -1) {
					$('#wine_label_check:first-child').remove();
					$('.upload_wine_label_limit').removeAttr('data-success').text('图片限于png,jpeg,jpg格式,请重选').css('color', '#EA0000');
